...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('fasilitass.update', $fasilitass->id_fasilitas) }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="id_hotel" id="id_hotel" required>
                            @foreach ($hotel as $p)
                                <option {{ $p->id_hotel == $hotelSaatIni->id_hotel ? 'selected' : '' }}
                                    value="{{ $p->id_hotel }}">
                                    {{ $p->nama }}
                                </option>
                            @endforeach
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_fasilitas" class="form-control" value="{{ $fasilitass->nama }}" id="basic-default-fullname"
                            placeholder="Masukkan Nama Fasilitas">
                    </div>
                    <div class="mb-3">
                        <label class="form-label" for="basic-default-fullname">Deskripsi</label>
                        <textarea class="form-control" id="exampleFormControlTextarea1" name="deskripsi">{{ $fasilitass->deskripsi }}</textarea>
                    </div>
